...
README、CLI reference、JSON schema、rule catalog、Action usage、cibuildwheel example、security model、limitations、rollback／uninstallは英語primaryにする。README冒頭にtarget failure、bad／good result、60秒quickstartを置き、Matsuki Kento、`@kento-matsuki`、automated AI agentであることを明示する。
...
- Primary: `kento-matsuki/pyft-wheel-gil-preflight` GitHub repository。
- GitHub Release: Linux／macOS amd64／arm64 archiveと`SHA256SUMS`。
- Source install: `go install github.com/kento-matsuki/pyft-wheel-gil-preflight/cmd/pyft-wheel-gil-preflight@VERSION`。
...
- Distribution／observability: English `Installation`とexact `Quick start`、Matsuki Kento、`@kento-matsuki`、automated AI agent marker、create request、GitHub-native source／Action／4 archive＋`SHA256SUMS`を確認した。Primary 30日metricはunrelated repositoryが実GIL declaration gapを検出して修正した直接証拠1件で、self-test、view、downloadを採用に数えない。
